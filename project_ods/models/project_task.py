...
class ProjectTask(models.Model):
    ...
    service_order_number = fields.Char(string="OdS")
    ...
    project_project_service_order_iho = fields.Boolean(related="project_id.service_order_iho")
    project_project_warehouse_order_iho = fields.Boolean(related="project_id.warehouse_order_iho")
    ods_oda_name = fields.Char(compute="_compute_ods_oda_name")

    @api.depends("project_project_warehouse_order_iho", "project_project_service_order_iho")
    ...
    def _compute_ptc_number_of_ids(self):
        for rec in self:
            rec.ptc_number_of_ids = (
                len(rec.project_task_class_ids.filtered(lambda l: l.warning_message).ids)
                if rec.project_task_class_ids
                else 0
            )
    ...
    def _compute_display_name(self):
        for rec in self:
            rec.display_name = (rec.service_order_number + " " if rec.service_order_number else "") + (
    ...
    def _compute_company_partner_id(self):
        for rec in self:
            rec.company_partner_id = rec.company_id.partner_id if rec.company_id.partner_id else False

    @api.depends("sale_order_id")
    def _compute_sale_order_opportunity_id(self):
        for rec in self:
            rec.sale_order_opportunity_id = (
                rec.sale_order_id.opportunity_id if rec.sale_order_id and rec.sale_order_id.opportunity_id else False
            )
    ...
    def _compute_ptc_service_type(self):
        for rec in self:
            ...
            for ptc in rec.project_task_class_ids:
                ptc_service_type += (
                    ptc.name + ";   "
                    if (ptc.parent_id == self.env.ref("project_ods.project_task_class_service_type"))
                    else ""
                )
    ...
    def _compute_ptc_additionals(self):
        for rec in self:
            ...
            for ptc in rec.project_task_class_ids:
                ptc_additionals += (
                    ptc.name + ";   "
                    if (ptc.parent_id == self.env.ref("project_ods.project_task_class_additionals"))
                    else ""
                )
    ...
    def _validate_requested_execution_date(self):
        if self.project_id.service_order_iho and self.requested_execution_date_time:
            ...
            non_working_days = []
            global_leave_ids = self.project_id.resource_calendar_id.global_leave_ids
            if global_leave_ids:
                for leave in global_leave_ids:
                    for span in range(0, (leave.date_to.date() - leave.date_from.date()).days):
                        non_working_days.append(leave.date_from.date() + date_utils.get_timedelta(span, "day"))
            # look for best possible execution day
            while (best_date.weekday() == 6) or (best_date in non_working_days):
                best_date = date_utils.add(best_date, days=1)
            if self.requested_execution_date_time.date() < best_date:
                raise ValidationError(_("Date not available, select on or after %s") % best_date.strftime("%d-%m-%Y"))

    def show_task_warnings(self):
        return self.project_task_class_ids.show_class_warnings(self.project_task_class_ids.ids)
            ...
            if rec.project_id.service_order_iho:
                if not rec.service_order_number:
                    if not rec.service_center_id.service_order_sequence_id:
                        raise ValidationError(_("Service center sequence not defined"))
                    vals["service_order_number"] = rec.service_center_id.service_order_sequence_id.next_by_id()
                vals["ptc_was_modified"] = False
            if rec.project_id.warehouse_order_iho:
                if not rec.service_order_number:
                    if not rec.service_center_id.warehouse_order_sequence_id:
                        raise ValidationError(_("Service center sequence not defined"))
                    vals["service_order_number"] = rec.service_center_id.warehouse_order_sequence_id.next_by_id()
                vals["ptc_was_modified"] = False
    ...
    def copy(self, default=None):
        if default is None:
            default = {}
        for rec in self:
            if rec.project_id.service_order_iho or rec.project_id.warehouse_order_iho:
                if rec.project_id.service_order_iho:
                    if not rec.service_center_id.service_order_sequence_id:
                        raise ValidationError(_("Service center sequence not defined"))
                    default["service_order_number"] = rec.service_center_id.service_order_sequence_id.next_by_id()
                if rec.project_id.warehouse_order_iho:
                    if not rec.service_center_id.warehouse_order_sequence_id:
                        raise ValidationError(_("Service center sequence not defined"))
                    default["service_order_number"] = rec.service_center_id.warehouse_order_sequence_id.next_by_id()
