...
class ReportAccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        columns = [
            ...
            {'name': _("Type of currency"), 'class': 'date', 'style': 'white-space:nowrap;'},
            {'name': _("Exchange rate"), 'class': 'date', 'style': 'white-space:nowrap;'},
            ...
            {'name': _("As of: %s") % format_date(self.env, options['date']['date_to']),
                'class': 'number sortable', 'style': 'white-space:nowrap;'},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.env['report.account.report_agedpartnerbalance'].with_context(
            **context)._get_partner_move_lines(
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    if aml.move_id.is_purchase_document():
                        for invoice_lines_ids in aml:
                            if invoice_lines_ids.account_id == aml.move_id.partner_id.property_account_payable_id:
                                if invoice_lines_ids.amount_currency:
                                    total_debit_credit = (
                                        invoice_lines_ids.debit - invoice_lines_ids.credit
                                        ) / invoice_lines_ids.amount_currency
                                    exchange_rate = "{:.4f}".format(total_debit_credit)
                                if aml.move_id.currency_id.name == 'MXN':
                                    exchange_rate = 1.0

                    if aml.move_id.is_sale_document():
                        for invoice_lines_ids in aml:
                            if invoice_lines_ids.account_id == aml.move_id.partner_id.property_account_receivable_id:
                                if invoice_lines_ids.amount_currency:
                                    total_debit_credit = (
                                        invoice_lines_ids.debit - invoice_lines_ids.credit
                                        ) / invoice_lines_ids.amount_currency
                    ...
                    vals = {
                        ...
                        'columns': [
                            {'name': v} for v in [
                                aml.move_id.ref, aml.move_id.currency_id.name,
                                aml.move_id.currency_id.symbol + str(exchange_rate),
                                ...
                                format_date(self.env, aml.expected_pay_date)]] + [{
                                    ...
                                    'no_format': sign * v
                                } for v in [line['period'] == 6-i and line['amount'] or 0 for i in range(7)]],
                        'action_context': {
