    ...
    def _compute_amount_untaxed_usd(self):
        for rec in self:
            rec.amount_untaxed_usd = rec.currency_id._convert(
                rec.amount_untaxed, self.env.ref("base.USD"), rec.company_id, rec.date_approve or datetime.date.today()
            )
    ...
    def _compute_amount_total_usd(self):
        for rec in self:
            rec.amount_total_usd = rec.currency_id._convert(
                rec.amount_total, self.env.ref("base.USD"), rec.company_id, rec.date_approve or datetime.date.today()
            )
