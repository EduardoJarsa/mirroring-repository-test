...
class SaleOrderLine(models.Model):
    ...
    iho_price_list = fields.Float(string="Price List", help="Vendor Catalog public price", default=1.0)
    ...
    def _onchange_customer_discount(self):
        if self.customer_discount < 0 or self.customer_discount > 100:
            raise ValidationError(_("Error: Customer discount must be [0-100]"))

    @api.onchange("iho_factor")
    def _onchange_iho_factor(self):
        if self.iho_factor < 1 or self.iho_factor > 10:
            raise ValidationError(_("Error: Factor must be [1-10]"))

    @api.onchange("iho_service_factor")
    def _onchange_iho_service_factor(self):
        if self.product_id.type in ("product", "consu") and (
            self.iho_service_factor < 1 or self.iho_service_factor > 1.99
        ):
            raise ValidationError(_("Error: Service factor must be [1-1.99]"))
    ...
    def _product_int_ref(self):
        ...
        if not int_ref or self.product_id == self.env.ref("sif_interface.product_product_dummy"):
        ...
        for rec in self:
            if rec.dealer_discount < 0 or rec.dealer_discount > 100:
                raise ValidationError(
                    _("Error: Column 'Dealer discount' at [%s] has value of [%s] and must be [0-100]")
    ...
    def _constrains_customer_discount(self):
        for rec in self:
            if rec.customer_discount < 0 or rec.customer_discount > 100:
                raise ValidationError(
                    _("Error: Column 'Customer discount' at [%s] has value of [%s] and must be [0-100]")
    ...
    def _constrains_iho_factor(self):
        for rec in self:
            if rec.iho_factor < 1 or rec.iho_factor > 10:
                raise ValidationError(
                    _("Error: Column 'Factor' at [%s] has value of [%s] and must be [1-10]")
    ...
    def _constrains_iho_service_factor(self):
        for rec in self:
            if rec.product_id.type in ("product", "consu") and (
                rec.iho_service_factor < 1 or rec.iho_service_factor > 1.99
            ):
                raise ValidationError(
                    _("Error: Column 'Service factor' at [%s] has value of [%s] and must be [1-1.99]")
                    % (rec._product_int_ref(), rec.iho_service_factor)
                )
            if rec.product_id.type == "service" and rec.iho_service_factor != 1.0:
    ...
    def _process_product_supplierinfo(self):
        for rec in self:
            ...
            if not rec.iho_currency_id:
                raise ValidationError(
                    _("Product Purchase Currency not defined for [%s]%s")
                    % (rec.product_id.default_code, rec.product_id.name)
                )
            context = {
                "partner": (rec.product_id.maker_id.id if rec.product_id.maker_id.id else rec.order_id.partner_id.id),
                "order": rec.order_id,
            }
            seller_line = rec.product_id.seller_ids.with_context(**context).filtered(
                lambda r: r.name.id == r._context.get("partner") and r.sale_order_id == r._context.get("order")
            )
            seller_to_create = {
                "name": (rec.product_id.maker_id.id if rec.product_id.maker_id.id else rec.order_id.partner_id.id),
                "delay": 1,
                "min_qty": 0,
    ...
    def _compute_is_bom_line__service_factor__iho_currency_id(self):
        for rec in self:
            ...
            if rec.product_id.maker_id.property_purchase_currency_id:
                rec.iho_currency_id = rec.product_id.maker_id.property_purchase_currency_id

    @api.depends("iho_price_list", "iho_factor")
    def _compute_sell_1(self):
        for rec in self:
            rec.iho_sell_1 = rec.iho_price_list * rec.iho_factor

    @api.depends("iho_price_list", "dealer_discount")
    def _compute_iho_purchase_cost(self):
        for rec in self:
            rec.iho_purchase_cost = rec.iho_price_list * (1 - rec.dealer_discount / 100)

    @api.depends("iho_sell_1", "customer_discount")
    def _compute_sell_2(self):
        for rec in self:
            rec.iho_sell_2 = rec.iho_sell_1 * (1 - rec.customer_discount / 100)
    ...
    @api.depends("product_id", "iho_price_list", "iho_factor", "customer_discount", "product_uom_qty")
    def _compute_discount_extended(self):
        for rec in self:
            rec.discount_extended = (
                -rec.iho_price_list * rec.iho_factor * rec.customer_discount / 100 * rec.iho_tc * rec.product_uom_qty
            )

    @api.depends("product_id", "iho_price_list", "iho_service_factor", "product_uom_qty")
    ...
    @api.depends("product_id", "iho_price_list", "product_uom_qty")
