...
class TestStockPicking(TransactionCase):
    ...
    def test_10_create_route_internal(self):
        ...
        picking_wh2 = pickings.filtered(
            lambda p: p.picking_type_id.warehouse_id.id == self.warehouse_2.id)
        picking_wh3 = pickings.filtered(
            lambda p: p.picking_type_id.warehouse_id.id == self.warehouse_3.id)
