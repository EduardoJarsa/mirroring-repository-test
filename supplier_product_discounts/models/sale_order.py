...
class SaleOrderLine(models.Model):
    ...
    vendor_id = fields.Many2one(
        "res.partner",
        string="Partner",
        domain=[("supplier", "=", True), ("ref", "!=", False), ("is_company", "=", True)],
    )
...
class SaleOrder(models.Model):
    ...
    discounts_ids = fields.One2many("vendor.sale.order.discounts", "sale_id", copy=False)
    ...
    def _validate_discounts(self):
        ...
        self._get_combinations_discounts(makers, catalogs, families, order_lines, combination_discounts)
        comb_disc_str = "\n"
        ...
        catalog_error = []
        general_discounts = self.env["vendor.product.discounts"].search([("id", "!=", False)])
        for catalog in combination_discounts:
            # This get the combination from SO
            discount = self._get_array_of_discounts(catalog, self.discounts_ids, general_discounts)
        ...
        if catalog_error:
            ...
            for rec in catalog_error:
                ...
                if rec["maker"] and rec["catalog"] and not rec["family"]:
                    message = _(
                        "{ maker %s catalog %s" "\n The offered discount is %s and the approved" " discount is %s}\n"
                    ) % (rec["maker"], rec["catalog"], rec["discount_offered"], rec["discount_accepted"])
                    msg = msg + message
                if rec["maker"] and rec["family"] and not rec["catalog"]:
                    message = _(
                        "{ maker %s family %s" "\n The offered discount is %s and the approved" " discount is %s}\n"
                    ) % (rec["maker"], rec["family"], rec["discount_offered"], rec["discount_accepted"])
                    msg = msg + message
                ...
                if rec["catalog"] and not rec["maker"] and not rec["family"]:
                    message = _(
                        "{ catalog %s" "\n the offered discount is %s and the approved" " discount is %s}\n"
                    ) % (rec["catalog"], rec["discount_offered"], rec["discount_accepted"])
                    msg = msg + message
                if rec["family"] and not rec["catalog"] and not rec["maker"]:
                    message = _(
                        "{ family %s" "\n the offered discount is %s and the approved" " discount is %s}\n"
                    ) % (rec["family"], rec["discount_offered"], rec["discount_accepted"])
                    msg = msg + message
            raise ValidationError(msg)

    def _get_three_filter(self, catalog, discounts, general_discounts):
        if catalog["maker_id"] and catalog["catalog_id"] and catalog["family_id"]:
    ...
    def _get_two_filter_comb_1(self, catalog, discounts_ids, general_discounts):
    ...
    def _filter_by_maker(self, catalog, discounts):
        filter_maker_id = discounts.filtered(
            lambda l: l.partner_id == catalog["maker_id"] and not l.catalog_id and not l.family_id
        )
        return filter_maker_id

    def _get_global_discounts(self, catalog, discounts):
        filter_global_discounts = discounts.filtered(
            lambda l: not l.partner_id and not l.catalog_id and not l.family_id
        )
        return filter_global_discounts

    def _get_array_of_discounts(self, catalog, discounts_ids, general_discounts):
        filter_discounts = False
        filter_discounts = self._get_three_filter(catalog, discounts_ids, general_discounts)
        if not filter_discounts:
            filter_discounts = self._get_two_filter_comb_1(catalog, discounts_ids, general_discounts)
        if not filter_discounts:
            filter_discounts = self._get_two_filter_comb_2(catalog, discounts_ids, general_discounts)
        if not filter_discounts:
            if catalog["maker_id"]:
                filter_maker_id = self._filter_by_maker(catalog, discounts_ids)
                if filter_maker_id:
                    return filter_maker_id
                gral_discounts = self._filter_by_maker(catalog, general_discounts)
                if gral_discounts:
                    return gral_discounts
            if not catalog["maker_id"] and not catalog["catalog_id"] and not catalog["family_id"]:
                global_discounts = self._get_global_discounts(catalog, discounts_ids)
                if global_discounts:
                    return global_discounts
                gral_discounts_global_disc = self._get_global_discounts(catalog, general_discounts)
                if gral_discounts_global_disc:
                    return gral_discounts_global_disc
        return filter_discounts

    def _compare_discounts(self, discount, catalog, catalog_error):
        ...
        if not success:
            ...
            if not exist:
                new_error_discount = {
                    ...
                    "discount_offered": "{percent:.2%}".format(percent=catalog["percentage"]),
                }
                if discount:
                    new_error_discount.update(
                        {"discount_accepted": "{percent:.2%}".format(percent=discount.discount / 100)}
                    )
                else:
                    new_error_discount.update({"discount_accepted": 0})
    ...
    def _filter_all_combinations(
        ...
        filter_tree_comb = order_lines.filtered(
            lambda l: l.vendor_id == maker and l.catalog_id == catalog and l.family_id == family
        )
        if filter_tree_comb:
            self._add_combination_discount(filter_tree_comb, combination_discounts, maker, catalog, family)
        # filter maker , catalog, not family
        filter_two_comb_1 = order_lines.filtered(
            lambda l: l.vendor_id == maker and l.catalog_id == catalog and not l.family_id
        )
        if filter_two_comb_1:
            self._add_combination_discount(filter_two_comb_1, combination_discounts, maker, catalog, False)
        # filter maker , family not catalog
        filter_two_comb_2 = order_lines.filtered(
            lambda l: l.vendor_id == maker and l.family_id == family and not l.catalog_id
        )
        if filter_two_comb_2:
            self._add_combination_discount(filter_two_comb_2, combination_discounts, maker, False, family)
        filter_maker_id = order_lines.filtered(lambda l: l.vendor_id == maker)
        # filter only maker
        if filter_maker_id:
            self._add_combination_discount(filter_maker_id, combination_discounts, maker, False, False)
        ...
        for maker in makers:
            for catalog in catalogs:
                for family in families:
                    self._filter_all_combinations(family, maker, catalog, combination_discounts, order_lines)

    def _add_combination_discount(self, sale_lines, combination_discounts, maker=False, catalog=False, family=False):
        subtotal = 0
        new_subtotal = 0
        percentage = 0
        if sale_lines:
            for order_line in sale_lines:
                subtotal = subtotal + (order_line.product_uom_qty) * (order_line.price_unit)
                new_subtotal = new_subtotal + (order_line.price_subtotal)
